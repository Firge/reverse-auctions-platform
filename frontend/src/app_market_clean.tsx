...
const dateText = (v?: string | null) =>
  !v ? "-" : new Intl.DateTimeFormat("ru-RU", { dateStyle: "medium", timeStyle: "short" }).format(new Date(v));
...
function Status({ s }: { s?: string }) {
  return <span className={`mk-status mk-status-${(s ?? "").toLowerCase()}`}>{statusText(s)}</span>;
}
function Tile({
  a,
  open,
  ...
  return (
    <div className="mk-auction-card-shell">
      <button type="button" className={`mk-auction-card${isActive ? " active" : ""}`} onClick={open}>
        <div className="mk-product-media" aria-hidden="true">
          <div className="mk-product-badge">Реверс</div>
          <div className="mk-product-icon">{(a.title || "A")[0].toUpperCase()}</div>
        </div>
        <div className="mk-auction-head">
          <span className="mk-id">#{a.id}</span>
          <Status s={a.status} />
        </div>
        <h4>{a.title}</h4>
        <p>{a.description || "Нет описания"}</p>
        <div className="mk-auction-meta">
          <span>{money(a.current_price ?? a.start_price)}</span>
          <span>{timeLeft(a.end_date, nowMs)}</span>
        </div>
      </button>
      {editAction ? (
        <div className="mk-card-inline-action">
          <button type="button" className="mk-ghost" onClick={editAction.onClick}>
            {editAction.label ?? "Редактировать"}
          </button>
        </div>
      ) : null}
    </div>
...
function LotPicker({
  ...
  const filteredItems = useMemo(() => {
    if (mode === "tree" || !query.trim()) return items;
    const lowerQuery = query.toLowerCase();
    return items.filter(
      (item) => item.name.toLowerCase().includes(lowerQuery) || item.code.toLowerCase().includes(lowerQuery),
    );
  }, [items, query, mode]);
  ...
  return (
    <div className="mk-lot-picker">
        ...
        <>
          ...
          <div className="mk-lot-picker-floating" role="dialog" aria-modal="true" aria-label="Каталог лотов">
            <div className="mk-subhead">
              ...
              <button type="button" className="mk-ghost" onClick={() => setCatalogOpen(false)}>
                Закрыть
              </button>
            </div>
            <div className="mk-tabs">
              <button
                type="button"
                className={mode === "search" ? "mk-tab active" : "mk-tab"}
              ...
              <button
                type="button"
                className={mode === "tree" ? "mk-tab active" : "mk-tab"}
            ...
            {mode === "search" ? (
              <label className="mk-field-label">
                Поиск лотов
                <input
                  value={query}
            ...
            {mode === "tree" && nodeId !== undefined && !items.length && !loading && !error ? (
              <div className="mk-note">
                Для выбранной категории нет прямых позиций. Попробуйте другую конечную категорию.
              </div>
            ) : null}

            {error ? <div className="mk-warning">{error}</div> : null}
            {loading ? <div className="mk-empty small">Загрузка каталога...</div> : null}
            {!loading && !filteredItems.length ? <div className="mk-empty small">Позиции не найдены.</div> : null}
      ...
      <div className="mk-subhead">
        <span>Выбранные лоты</span>
        <small>{selectedLots.length}</small>
      </div>
              ...
              <div key={lot.id} className="mk-selected-lot-row">
                ...
                <div className="mk-selected-lot-controls">
                  <input
                    ...
                    disabled={disabled}
                  />
                  <button type="button" className="mk-ghost" onClick={() => removeLot(lot.id)} disabled={disabled}>
                    Удалить
                  </button>
                </div>
              </div>
...
export function App() {
  ...
  const [createLotsDetails, setCreateLotsDetails] = useState<Record<number, CatalogItem>>({});
  ...
  const tokenUserId =
    typeof claims?.user_id === "number"
      ...
      : typeof claims?.user_id === "string" && claims.user_id.trim()
        ...
        : undefined;
  const userId = Number.isFinite(tokenUserId)
    ? tokenUserId
  ...
  async function loadAuction(id: number, silent = false) {
    ...
    try {
      setAuction(normalizeAuction(await fetchAuction(id, apiBase, tokens?.access ?? undefined)));
    } catch (e) {
      setAuction(null);
      setToast({ kind: "error", text: `Не удалось загрузить аукцион: ${toRussianErrorMessage(e)}` });
    } finally {
      if (!silent) setLoadingAuction(false);
    }
  }
  async function loadBids(id: number) {
    if (!tokens?.access) {
      setBids([]);
      setBidsMsg("Войдите, чтобы посмотреть историю ставок.");
      return;
    }
    setLoadingBids(true);
    setBidsMsg("");
    try {
      const data = await fetchAuctionBids(id, tokens.access, apiBase);
      setBids(data);
      if (!data.length) setBidsMsg("Ставок пока нет.");
    } catch (e) {
      setBids([]);
      setBidsMsg(toRussianErrorMessage(e));
    } finally {
  ...
  async function loadWinner(id: number) {
    if (!tokens?.access) {
      setWinner(null);
      setWinnerMsg("Войдите, чтобы проверить победителя.");
      return;
    }
    setLoadingWinner(true);
    setWinnerMsg("");
    try {
      setWinner(await fetchAuctionWinner(id, tokens.access, apiBase));
    } catch (e) {
      setWinner(null);
      setWinnerMsg(toRussianErrorMessage(e));
    } finally {
  ...
  useEffect(() => {
    ...
    const t = window.setInterval(() => {
      void syncServerTime();
      void refreshAuctions();
      if (selectedId) void loadAuction(selectedId, true);
    }, 10000);
    return () => window.clearInterval(t);
  }, [apiBase, selectedId]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    auction?.start_date,
    auction?.end_date,
    auction?.specific?.min_bid_decrement,
  ]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (route.name === "auction" && detailTab === "bids" && selectedId) void loadBids(selectedId);
  }, [route, detailTab, selectedId, apiBase, tokens?.access]);
  useEffect(() => {
  ...
  useEffect(() => {
    let active = true;
    const loadLotsDetails = async () => {
      const ids = createLots.map((lot) => lot.id);
      if (!ids.length) {
        setCreateLotsDetails({});
        return;
      }
      try {
        const resolved = await fetchCatalogItemsByIds(ids, apiBase, tokens?.access ?? undefined);
        if (!active) return;
        const byId: Record<number, CatalogItem> = {};
        resolved.forEach((item) => {
          byId[item.id] = item;
        });
        setCreateLotsDetails(byId);
      } catch {
        if (active) setCreateLotsDetails({});
      }
    };
    void loadLotsDetails();
    return () => {
      active = false;
    };
  }, [createLots, apiBase, tokens?.access]);
  ...
  async function onLogin(e: FormEvent) {
    ...
    setAuthLoading(true);
    try {
      const pair = await loginUser(loginForm.username, loginForm.password, apiBase);
      setStoredTokens(pair);
      setTokens(pair);
      setToast({ kind: "ok", text: "Вход выполнен." });
      go({ name: "account" });
    } catch (err) {
      setToast({ kind: "error", text: `Ошибка входа: ${toRussianErrorMessage(err)}` });
    } finally {
  ...
  async function onRegister(e: FormEvent) {
    ...
    try {
      const res = await registerUser(
        { ...registerForm, role: registerForm.role === "admin" ? "supplier" : registerForm.role },
        apiBase,
      );
      setLoginForm((f) => ({ ...f, username: res.username }));
      setToast({ kind: "ok", text: `Аккаунт создан: ${res.username}.` });
      go({ name: "login" });
    } catch (err) {
      setToast({ kind: "error", text: `Ошибка регистрации: ${toRussianErrorMessage(err)}` });
    } finally {
      setAuthLoading(false);
    }
  }
  ...
  function calculateAveragePrice() {
    if (!createLots.length) {
      setPriceCalcMsg("Выберите минимум один лот.");
      return;
    }
    const prices: number[] = [];
    let missingCount = 0;
    ...
    if (!prices.length) {
      setPriceCalcMsg(`Информация о стоимости отсутствует для ${missingCount} лот(а). Установите цену вручную.`);
      return;
    }
    const average = prices.reduce((a, b) => a + b, 0) / prices.length;
    const rounded = Math.round(average * 100) / 100;
    setCreateForm((f) => ({ ...f, start_price: String(rounded) }));
    setPriceCalcMsg(
      missingCount
        ? `Средняя стоимость ${rounded} ₽ (рассчитана по ${prices.length} лотам). ${missingCount} лот(а) не имеют цены.`
        : `Средняя стоимость ${rounded} ₽`,
    );
  ...
  async function onCreate(e: FormEvent) {
    e.preventDefault();
    if (!tokens?.access) {
      setToast({ kind: "error", text: "Сначала войдите в аккаунт, чтобы создать аукцион." });
      return;
    }
    ...
    try {
      const created = normalizeAuction(await createAuction(payload, tokens.access, apiBase));
      setCreateForm(DEFAULT_CREATE);
      setCreateLots([]);
      setToast({ kind: "ok", text: `Аукцион создан.` });
      await refreshAuctions();
      await loadOwnedAuctions(tokens.access);
      openAuction(created.id);
    } catch (err) {
      setToast({ kind: "error", text: `Ошибка создания: ${toRussianErrorMessage(err)}` });
    } finally {
  ...
  async function onBid(e: FormEvent) {
    ...
    if (!auction) return;
    if (!tokens?.access) {
      setToast({ kind: "error", text: "Сначала войдите в аккаунт, чтобы сделать ставку." });
      go({ name: "login" });
      return;
    }
    setBidLoading(true);
    try {
      const result = await submitBid(auction.id, tokens.access, bidForm, apiBase);
      window.location.href = result.redirect_url;
    } catch (err) {
      setToast({ kind: "error", text: `Ошибка ставки: ${toRussianErrorMessage(err)}` });
    } finally {
      setBidLoading(false);
    }
  }
  ...
  async function onSignConfirmation(role: "creator" | "winner") {
    ...
    try {
      ...
      setToast({
        kind: "ok",
        text: role === "creator" ? "Подтверждение заказчика отправлено." : "Подтверждение победителя отправлено.",
      });
    } catch (err) {
  ...
  const canOwnerCloseAuction =
    !!auction && isAuctionOwner && !["CLOSED", "FINISHED", "CANCELED"].includes(auction.status);
  ...
  return (
    <div className="mk-shell">
      <header className="mk-topbar mk-topbar-market">
        <button type="button" className="mk-brand mk-brand-btn" onClick={() => go({ name: "home" })}>
          <div className="mk-logo">BF</div>
          <div>
            <strong>Bidfall Маркет</strong>
            <span>Активные аукционы</span>
          </div>
        </button>
        <div className="mk-searchbar mk-searchbar-market">
          <input
            placeholder="Поиск аукционов по названию, ID или статусу"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            onKeyDown={(e) => {
              if (e.key === "Enter") go({ name: "browse" });
            }}
          />
          <button type="button" onClick={() => go({ name: "browse" })}>
            Поиск
          </button>
        </div>
        <button
        ...
        <nav className={`mk-nav${navOpen ? " open" : ""}`}>
          ...
          <button
            type="button"
            className={navActive("browse") ? "mk-pill active" : "mk-pill"}
          ...
          {tokens?.access ? (
            <button
              type="button"
              className={navActive("account") ? "mk-pill active" : "mk-pill"}
          ...
          {!tokens?.access ? (
            <button
              type="button"
              className={route.name === "login" || route.name === "register" ? "mk-pill active" : "mk-pill"}
        ...
        <>
          <div className="mk-hero mk-hero-market">
            <div className="mk-hero-copy">
              <h1>
                Умные закупки через <em>реверсный аукцион</em>
              </h1>
              <p>
                Поставщики соревнуются ценой — вы получаете лучшие условия. Прозрачная торговая площадка для B2B закупок
              ...
              <div className="mk-hero-cta">
                <button type="button" className="mk-btn-amber" onClick={() => go({ name: "browse" })}>
                  Просмотреть аукционы
                </button>
                <button type="button" className="mk-ghost" onClick={() => go({ name: "sell" })}>
                  Создать аукцион
                </button>
              </div>
            </div>
            <div className="mk-hero-stats">
              <div>
                <small>Идёт сейчас</small>
                <strong>{activeAuctions.length}</strong>
              </div>
              <div>
                <small>Всего на платформе</small>
                <strong>{allAuctions.length}</strong>
              </div>
              <div>
                <small>Формат</small>
                <strong>Реверсный</strong>
              </div>
              <div>
                <small>Валюта</small>
                <strong>₽</strong>
          ...
          <Card
            title="Идущие сейчас аукционы"
            subtitle="Аукционы, которые идут прямо сейчас."
            action={
              <button type="button" className="mk-ghost" onClick={() => go({ name: "browse" })}>
                Показать все
              </button>
            }
          >
            {loadingAuctions ? <div className="mk-empty">Загрузка аукционов...</div> : null}
            {!loadingAuctions && !liveHome.length ? (
              <div className="mk-empty">Сейчас нет активных аукционов.</div>
            ) : null}
            {!!liveHome.length ? (
              <div className="mk-grid mk-grid-market">
                {liveHome.map((a) => (
                  <Tile key={a.id} a={a} nowMs={serverNowMs} open={() => openAuction(a.id)} />
                ))}
              </div>
            ) : null}
          </Card>
          <Card title="Скоро завершатся" subtitle="Быстрый доступ к аукционам, которые закроются первыми.">
            <div className="mk-list-rows">
              {endingSoon.map((a) => (
                <button key={a.id} type="button" className="mk-row-link" onClick={() => openAuction(a.id)}>
                  <div>
                    <strong>{a.title}</strong>
                    <span>{statusText(a.status)}</span>
                  </div>
                  <div>
                    <strong>{money(a.current_price ?? a.start_price)}</strong>
                    <span>{timeLeft(a.end_date, serverNowMs)}</span>
                  </div>
                </button>
              ))}
      ...
      {route.name === "browse" ? (
        <div className="mk-page-grid">
          <aside className="mk-page-sidebar">
            <Card title="Фильтры" subtitle="Параметры отображения списка аукционов.">
              <div className="mk-filter-block">
                <div className="mk-tabs">
                  <button
                    type="button"
                    className={catalogMode === "all" ? "mk-tab active" : "mk-tab"}
                  ...
                  <button
                    type="button"
                    className={catalogMode === "active" ? "mk-tab active" : "mk-tab"}
                ...
                <label className="mk-field-label">
                  ...
                  <input
                    value={query}
                    onChange={(e) => setQuery(e.target.value)}
                    placeholder="Название аукциона, статус, ID"
                  />
                </label>
                <div className="mk-note">{filtered.length} объявлений</div>
              </div>
            </Card>
          </aside>
          <section className="mk-page-content">
            <Card
              ...
              subtitle="Просмотр всех аукционов в формате витрины."
              action={
                <div>
                  {loadingAuctions && <small className="mk-inline-muted">Обновление...</small>}
                  {tokens?.access && (
                    <button type="button" className="mk-ghost" onClick={() => go({ name: "sell" })}>
                      Создать аукцион
                    </button>
            ...
            >
              {loadingAuctions && !filtered.length ? <div className="mk-empty">Загрузка аукционов...</div> : null}
              {!loadingAuctions && !filtered.length ? (
                <div className="mk-empty">По вашему запросу ничего не найдено.</div>
              ) : null}
              {!!filtered.length ? (
                <div className="mk-grid mk-grid-market">
                  {filtered.map((a) => (
      ...
      {route.name === "auction" ? (
        <div className="mk-detail-layout">
          <section className="mk-detail-main">
            <Card
              title={auction?.title || "Аукцион"}
              subtitle={auction ? `Активный аукцион` : "Выберите аукцион из списка."}
              action={auction ? <Status s={auction.status} /> : undefined}
            >
              {loadingAuction ? <div className="mk-empty">Загрузка аукциона...</div> : null}
              {!loadingAuction && !auction ? <div className="mk-empty">Аукцион не найден.</div> : null}
              {auction ? (
                <div className="mk-detail">
                  <div className="mk-product-hero">
                    <div className="mk-product-gallery" aria-hidden="true">
                      <div className="mk-product-gallery-main">{(auction.title || "А")[0].toUpperCase()}</div>
                      <div className="mk-product-gallery-row">
                        <span />
                        <span />
                        <span />
                      </div>
                    </div>
                    <div className="mk-product-summary">
                      <p className="mk-product-copy">{auction.description || "Описание отсутствует."}</p>
                      <div className="mk-detail-stats">
                        <div>
                          <small>Текущая цена</small>
                          <strong>{money(auction.current_price ?? auction.start_price)}</strong>
                        </div>
                        <div>
                        ...
                        <div>
                          <small>До окончания</small>
                          <strong>{timeLeft(auction.end_date, serverNowMs)}</strong>
                        </div>
                        <div>
                          <small>Мин. шаг снижения</small>
                          <strong>{auction.specific?.min_bid_decrement ?? "-"}</strong>
                        </div>
                      </div>
                      <div className="mk-meta-list">
                        <span>Начало: {dateText(auction.start_date)}</span>
                        <span>Окончание: {dateText(auction.end_date)}</span>
                        <span>Продавец</span>
                      </div>
                    </div>
                  </div>
                  <div className="mk-tabs compact mk-segmented">
                    <button
                      type="button"
                      className={detailTab === "overview" ? "mk-tab active" : "mk-tab"}
                    ...
                    <button
                      type="button"
                      className={detailTab === "bids" ? "mk-tab active" : "mk-tab"}
                      onClick={() => {
                        setDetailTab("bids");
                        void loadBids(auction.id);
                      }}
                    >
                      Ставки
                    </button>
                  ...
                  {detailTab === "overview" ? (
                    <div className="mk-lots">
                      <div className="mk-subhead">
                        <span>Лоты</span>
                        <small>{auction.lots?.length ?? 0}</small>
                      </div>
                      {(auction.lots ?? []).map((lot) => (
                        <div key={`${lot.id}-${lot.code}`} className="mk-lot-row">
                          <div>
                            <strong>{lot.name}</strong>
                            <span>{lot.code || "Позиция"}</span>
                          </div>
                          <div>
                      ...
                      {!auction.lots?.length ? <div className="mk-empty small">Лоты пока не добавлены.</div> : null}
                    </div>
                  ) : (
                    <div className="mk-bids">
                      {bidsMsg ? <div className="mk-warning">{bidsMsg}</div> : null}
                      {loadingBids ? <div className="mk-empty small">Загрузка ставок...</div> : null}
                      {!loadingBids &&
                        bids.map((b) => (
                          <div key={b.id} className="mk-bid-row">
                            <div>
                              <strong>{money(b.bid)}</strong>
                              <span>Поставщик</span>
                            </div>
                            <p>{b.comment || "Без комментария"}</p>
                          </div>
                        ))}
                      {!loadingBids && !bids.length && !bidsMsg ? (
                        <div className="mk-empty small">Ставок пока нет.</div>
                      ) : null}
                    </div>
                  )}
                </div>
          ...
          <aside className="mk-detail-side">
            <Card
              title={isOwnerDraft ? "Редактирование черновика" : "Сделать ставку"}
              subtitle={
                isOwnerDraft
                  ? "Редактировать можно только черновики. Когда все готово, опубликуйте аукцион."
                  : "Отправьте новую ставку для этого аукциона."
              }
            >
              {!auction ? (
                <div className="mk-empty small">Сначала выберите аукцион.</div>
              ) : isOwnerDraft ? (
                <div className="mk-form">
                  <div className="mk-form-grid">
                    <label className="mk-field-label">
                      ...
                      <input
                        ...
                        onChange={(e) => setDraftEditForm((f) => ({ ...f, title: e.target.value }))}
                      />
                    </label>
                    <label className="mk-field-label">
                      Начальная цена
                      <input
                        type="number"
                        step="0.01"
                        value={draftEditForm.start_price}
                        onChange={(e) => setDraftEditForm((f) => ({ ...f, start_price: e.target.value }))}
                      />
                    </label>
                    <label className="mk-field-label">
                      Время начала
                      <input
                        type="datetime-local"
                        value={draftEditForm.start_date_local}
                        onChange={(e) => setDraftEditForm((f) => ({ ...f, start_date_local: e.target.value }))}
                      />
                    </label>
                    <label className="mk-field-label">
                      Время окончания
                      <input
                        type="datetime-local"
                        value={draftEditForm.end_date_local}
                        onChange={(e) => setDraftEditForm((f) => ({ ...f, end_date_local: e.target.value }))}
                      />
                    </label>
                    <label className="mk-field-label">
                      Мин. шаг снижения (%)
                      <input
                        type="number"
                        step="0.01"
                        value={draftEditForm.min_bid_decrement}
                        onChange={(e) => setDraftEditForm((f) => ({ ...f, min_bid_decrement: e.target.value }))}
                        placeholder="0.5"
                      />
                    </label>
                  </div>
                  ...
                  <LotPicker
                    selectedLots={draftLots}
                    onChange={setDraftLots}
                    baseUrl={apiBase}
                    token={tokens?.access ?? undefined}
                    disabled={draftEditLoading}
                  />
                  {draftLotErrors.length ? <div className="mk-warning">{draftLotErrors.join("; ")}</div> : null}
                  <div className="mk-inline-actions">
                    <button type="button" disabled={draftEditLoading} onClick={onSaveDraft}>
                      {draftEditLoading ? "Сохранение..." : "Сохранить черновик"}
                    </button>
                    <button type="button" className="mk-ghost" disabled={draftEditLoading} onClick={onPublish}>
                      {draftEditLoading ? "Подождите..." : "Опубликовать"}
                    </button>
                    {canOwnerCloseAuction ? (
                      <button
                        ...
                        className="mk-ghost"
                        disabled={draftEditLoading}
                        onClick={() => void onCloseAuction()}
                      >
                        {draftEditLoading ? "Подождите..." : "Закрыть аукцион"}
                      </button>
                    ) : null}
                  </div>
                </div>
              ) : isAuctionOwner ? (
                <div className="mk-empty small">
                  Этот аукцион уже опубликован. Владелец не может редактировать его или делать ставки.
                </div>
              ) : !tokens?.access ? (
                <div className="mk-empty small">Войдите как поставщик, чтобы делать ставки.</div>
              ) : !canBidByRole ? (
                <div className="mk-empty small">Ставки в этом аукционе могут делать только поставщики.</div>
              ) : auction.status !== "ACTIVE" ? (
                <div className="mk-empty small">Прием ставок начнется, когда аукцион станет активным.</div>
              ) : (
                <form className="mk-form" onSubmit={onBid}>
                  <label className="mk-field-label">
                    Цена ставки (чем ниже, тем лучше)
                    <input
                      type="number"
                      step="0.01"
                      min="0"
                      ...
                      placeholder="Введите более низкую цену"
                    />
                  </label>
                  <label className="mk-field-label">
                    ...
                    <input
                      ...
                      onChange={(e) => setBidForm((f) => ({ ...f, comment: e.target.value }))}
                      placeholder="Срок поставки, примечания"
                    />
                  </label>
                  <div className="mk-inline-actions">
                    <button type="submit" disabled={bidLoading}>
                      {bidLoading ? "Отправка..." : "Отправить ставку"}
                    </button>
                    <button type="button" className="mk-ghost" onClick={() => void loadBids(auction.id)}>
                      Обновить ставки
                    </button>
                    <button type="button" className="mk-ghost" onClick={() => void loadWinner(auction.id)}>
                      Проверить победителя
                    </button>
                  </div>
                  {loadingWinner || winner || winnerMsg ? (
                    <div className="mk-winner-box">
                      {loadingWinner ? <span>Проверка победителя...</span> : null}
                      {winner ? <span>Текущий победитель: {money(winner.bid)}</span> : null}
                      {!winner && winnerMsg ? <span>{winnerMsg}</span> : null}
                    </div>
                  ) : null}
                </form>
              )}
            ...
            {canSignResult ? (
              <Card
                title="Подписание результата ЭЦП"
                subtitle="Сформируйте текст подтверждения, подпишите его через КриптоПро и отправьте на сервер."
              >
                <div className="mk-form">
                  <div className="mk-inline-actions">
                    <button
                      type="button"
                      className="mk-ghost"
                      onClick={() => void loadConfirmation(auction!.id)}
                      disabled={loadingConfirmation}
                    >
                      {loadingConfirmation ? "Обновление..." : "Обновить статус"}
                    </button>
                    <button
                      type="button"
                      className="mk-ghost"
                      onClick={() => void onLoadCertificates()}
                      disabled={loadingCertificates}
                    >
                      {loadingCertificates ? "Загрузка..." : "Загрузить сертификаты"}
                    </button>
                  </div>
                  {confirmation ? (
                    <div className="mk-note">
                      Статус: {confirmation.status ?? "-"}. Заказчик:{" "}
                      {confirmation.creator_signed_at ? "подписал" : "не подписал"}, победитель:{" "}
                      {confirmation.winner_signed_at ? "подписал" : "не подписал"}. Дедлайн:{" "}
                      {dateText(confirmation.signing_deadline)}.
                    </div>
                  ) : null}
                  {confirmationMsg ? <div className="mk-warning">{confirmationMsg}</div> : null}
                  <label className="mk-field-label">
                    Сертификат
                    <select value={selectedCertThumbprint} onChange={(e) => setSelectedCertThumbprint(e.target.value)}>
                      <option value="">Выберите сертификат</option>
                      {certificates.map((cert) => (
                        <option key={cert.thumbprint} value={cert.thumbprint}>
                          {cert.subjectName} · до {dateText(cert.validTo)}
                        </option>
                      ))}
                    </select>
                  </label>
                  {canSignAsCreator || canSignAsWinner ? (
                    <div className="mk-inline-actions">
                      {canSignAsCreator ? (
                        <button
                          type="button"
                          onClick={() => void onSignConfirmation("creator")}
                          disabled={signingNow || !selectedCertThumbprint}
                        >
                          {signingNow ? "Подписание..." : "Подписать как заказчик"}
                        </button>
                      ) : null}
                      {canSignAsWinner ? (
                        <button
                          type="button"
                          onClick={() => void onSignConfirmation("winner")}
                          disabled={signingNow || !selectedCertThumbprint}
                        >
                          {signingNow ? "Подписание..." : "Подписать как победитель"}
                        </button>
                      ) : null}
                    </div>
                  ) : (
                    <div className="mk-note">Подписывать результат могут только заказчик или победитель аукциона.</div>
                  )}
                </div>
              </Card>
            ) : null}
            {isAuctionOwner && !isOwnerDraft && canOwnerCloseAuction ? (
              <Card title="Действия владельца" subtitle="Управление статусом аукциона.">
                <div className="mk-inline-actions">
                  <button type="button" className="mk-ghost" onClick={() => void onCloseAuction()}>
                    Закрыть аукцион
                  </button>
                </div>
              </Card>
            ) : null}
            <Card title="Другие аукционы" subtitle="Откройте другой активный или недавний аукцион.">
              <div className="mk-list-rows">
                {(activeAuctions.length ? activeAuctions : allAuctions).slice(0, 6).map((a) => (
                  <button key={a.id} type="button" className="mk-row-link" onClick={() => openAuction(a.id)}>
                    <div>
                      <strong>{a.title}</strong>
                      <span>{statusText(a.status)}</span>
                ...
                {!(activeAuctions.length || allAuctions.length) ? (
                  <div className="mk-empty small">Нет доступных аукционов.</div>
                ) : null}
              </div>
            </Card>
          </aside>
        ...
        <div className="mk-create-page-full">
          {!tokens?.access ? (
            <Card title="Нужен вход" subtitle="Чтобы создать аукцион, войдите в аккаунт.">
              ...
              <div className="mk-inline-actions">
                <button type="button" onClick={() => go({ name: "login" })}>
                  Перейти ко входу
                </button>
                <button type="button" className="mk-ghost" onClick={() => go({ name: "home" })}>
                  На главную
                </button>
              </div>
            </Card>
            ...
            <>
              <Card
                ...
                subtitle="Заполните параметры аукциона. Можно сохранить черновик или опубликовать."
              >
                <form className="mk-form" onSubmit={onCreate}>
                  <div className="mk-form-grid">
                    <label className="mk-field-label">
                      ...
                      <input
                        ...
                        placeholder="Например: закупка расходных материалов"
                      />
                    </label>
                    <label className="mk-field-label">
                      Начальная цена
                      <input
                        type="number"
                        step="0.01"
                        value={createForm.start_price}
                        onChange={(e) => setCreateForm((f) => ({ ...f, start_price: e.target.value }))}
                      />
                    </label>
                    <label className="mk-field-label">
                      Время начала
                      <input
                        type="datetime-local"
                        value={createForm.start_date_local}
                        onChange={(e) => setCreateForm((f) => ({ ...f, start_date_local: e.target.value }))}
                      />
                    </label>
                    <label className="mk-field-label">
                      Время окончания
                      <input
                        type="datetime-local"
                        value={createForm.end_date_local}
                        onChange={(e) => setCreateForm((f) => ({ ...f, end_date_local: e.target.value }))}
                      />
                    </label>
                    <label className="mk-field-label">
                      Мин. шаг снижения (%)
                      <input
                        type="number"
                        step="0.01"
                        value={createForm.min_bid_decrement}
                        onChange={(e) => setCreateForm((f) => ({ ...f, min_bid_decrement: e.target.value }))}
                        placeholder="0.5"
                      />
                    </label>
                    <label className="mk-field-label">
                      Тип аукциона
                      <input value="Реверсный аукцион" readOnly />
                    </label>
                  </div>
                  <label className="mk-field-label">
                    Описание
                    <textarea
                      ...
                      placeholder="Условия поставки, требования к участникам, комментарии"
                    />
                  </label>
                  <LotPicker
                    selectedLots={createLots}
                    onChange={setCreateLots}
                    baseUrl={apiBase}
                    token={tokens?.access ?? undefined}
                  ...
                  <div className="mk-inline-actions">
                    <button type="submit" disabled={createLoading}>
                      {createLoading ? "Сохранение..." : "Создать аукцион"}
                    </button>
                    <button type="button" className="mk-ghost" onClick={() => setCreateForm(DEFAULT_CREATE)}>
                      Сбросить форму
                    </button>
                  </div>
                </form>
              ...
              {showPriceCalc && (
                <>
                  <div className="mk-lot-picker-floating-backdrop" onClick={() => setShowPriceCalc(false)} />
                  <div
                    className="mk-lot-picker-floating"
                    role="dialog"
                    aria-modal="true"
                    aria-label="Расчет средней стоимости"
                  >
                    <div className="mk-subhead">
                      <span>Расчет средней стоимости</span>
                      <button type="button" className="mk-ghost" onClick={() => setShowPriceCalc(false)}>
                        Закрыть
                      </button>
                    </div>
                    <div className="mk-note">
                      На основе выбранных лотов рассчитаем среднюю стоимость, которая станет начальной ценой аукциона.
                    </div>
                    {createLots.length ? (
                      <div className="mk-lot-picker">
                        {createLots.map((lot) => {
                    ...
                    {priceCalcMsg ? <div className="mk-note">{priceCalcMsg}</div> : null}
                    <div className="mk-inline-actions">
                      <button type="button" onClick={() => calculateAveragePrice()} className="mk-ghost">
                        Рассчитать
                      </button>
                    </div>
                  </div>
      ...
      {route.name === "login" || route.name === "register" ? (
        <div className={route.name === "register" ? "mk-auth-page-full" : "mk-page-centered mk-auth-page"}>
          <Card
            ...
            subtitle={
              authMode === "login"
                ? "Войдите в аккаунт, чтобы делать ставки и управлять аукционами."
                : "Зарегистрируйте аккаунт участника и начните работу с аукционами."
            }
          >
            <div className="mk-auth">
              <div className="mk-tabs">
                <button
                  type="button"
                  className={authMode === "login" ? "mk-tab active" : "mk-tab"}
                  onClick={() => {
                    setAuthMode("login");
                    go({ name: "login" });
                  }}
                ...
                <button
                  type="button"
                  className={authMode === "register" ? "mk-tab active" : "mk-tab"}
                  onClick={() => {
                  ...
                  <div className="mk-form-grid">
                    ...
                    <label className="mk-field-label">
                      ...
                      <input
                        ...
                        inputMode="numeric"
                        pattern={INN_INPUT_PATTERN}
                        onChange={(e) => setRegisterForm((f) => ({ ...f, inn: normalizeInnInput(e.target.value) }))}
                      />
                    </label>
                  </div>
                  {registerInnLoading ? <div className="mk-note">Проверяем ИНН в DaData...</div> : null}
                  {registerInnResolved ? (
                    <div className="mk-note">Организация найдена: {registerInnResolved}</div>
                  ) : null}
        ...
        <div className="mk-account-page-full">
          <section className="mk-page-content">
            <Card
              ...
              subtitle="Основная информация профиля и краткая статистика по вашим аукционам."
            >
              <div className="mk-account-grid mk-account-grid-profile">
                ...
                <div className="mk-kv">
                  <span>Роль</span>
                  <strong>{roleText(currentUser?.profile?.role ?? "-")}</strong>
                </div>
                <div className="mk-kv">
                  <span>Компания</span>
                  <strong>{currentUser?.profile?.company_name || "-"}</strong>
                </div>
                <div className="mk-kv">
                  <span>ИНН</span>
                  <strong>{currentUser?.profile?.inn || "-"}</strong>
                </div>
                <div className="mk-kv">
                  <span>Рейтинг</span>
                  <strong>{currentUser?.profile?.rating ?? "-"}</strong>
                </div>
                <div className="mk-kv">
                  <span>ID пользователя</span>
                  <strong>{currentUser?.id ?? userId ?? "-"}</strong>
                </div>
                <div className="mk-kv">
                  <span>Статус сессии</span>
                  <strong>{tokens?.access ? "Выполнен вход" : "Гость"}</strong>
                </div>
                <div className="mk-kv">
                  <span>Мои аукционы</span>
                  <strong>{myAuctions.length}</strong>
                </div>
                <div className="mk-kv">
                  <span>Участвую в</span>
                  <strong>{participatingAuctions.length}</strong>
                </div>
              </div>
              {currentUserLoading && tokens?.access ? (
                <div className="mk-empty small">Загрузка данных аккаунта...</div>
              ) : null}
              <div className="mk-inline-actions">
                {!tokens?.access ? (
                  <button type="button" onClick={() => go({ name: "login" })}>
                ...
                {tokens?.access ? (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setOwnedAuctions([]);
                      setParticipatingAuctions([]);
                      setToast({ kind: "ok", text: "Вы вышли из аккаунта." });
                    }}
                  >
                    Выйти
                  </button>
            ...
            <Card
              ...
              subtitle="Изменяйте данные профиля. Эл. почта доступна только для чтения."
            >
                ...
                <form className="mk-form" onSubmit={onSaveAccount}>
                  <div className="mk-form-grid">
                    ...
                    <label className="mk-field-label">
                      ...
                      <input
                        ...
                        inputMode="numeric"
                        pattern={INN_INPUT_PATTERN}
                        onChange={(e) => setAccountForm((f) => ({ ...f, inn: normalizeInnInput(e.target.value) }))}
                      />
                    </label>
                    <label className="mk-field-label">
                  ...
                  <div className="mk-inline-actions">
                    <button type="submit" disabled={accountSaving}>
                      {accountSaving ? "Сохранение..." : "Сохранить изменения"}
                    </button>
                    <button
                      type="button"
                      className="mk-ghost"
            ...
            <Card title="Мои аукционы" subtitle="Ваши созданные аукционы. Черновики видны только вам.">
              ...
              {ownedAuctionsLoading && tokens?.access ? (
                <div className="mk-empty small">Загрузка аукционов...</div>
              ) : null}
              {!ownedAuctionsLoading && tokens?.access && !myAuctions.length ? (
                <div className="mk-empty">У этого аккаунта пока нет созданных аукционов.</div>
              ) : null}
              {!!myAuctions.length ? (
                <div className="mk-grid mk-grid-market">
                  {myAuctions.map((a) => (
                    <Tile
                      ...
                      a={a}
                      nowMs={serverNowMs}
                      open={() => openAuction(a.id)}
                      editAction={
                        a.status === "DRAFT" ? { onClick: () => openAuction(a.id), label: "Редактировать" } : undefined
                      }
                    />
                  ))}
                </div>
            ...
            <Card title="Аукционы, в которых вы участвуете" subtitle="Список аукционов, где вы уже сделали ставку.">
              {!tokens?.access ? (
                <div className="mk-empty">Войдите, чтобы увидеть аукционы с вашим участием.</div>
              ) : null}
              {participatingLoading && tokens?.access ? (
                <div className="mk-empty small">Загрузка аукционов...</div>
              ) : null}
              {!participatingLoading && tokens?.access && !participatingAuctions.length ? (
                <div className="mk-empty">Пока нет аукционов с вашим участием.</div>
              ) : null}
              {!!participatingAuctions.length ? (
                <div className="mk-grid mk-grid-market">
                  {participatingAuctions.map((a) => (
                    <Tile key={a.id} a={a} nowMs={serverNowMs} open={() => openAuction(a.id)} />
                  ))}
                </div>
              ) : null}
